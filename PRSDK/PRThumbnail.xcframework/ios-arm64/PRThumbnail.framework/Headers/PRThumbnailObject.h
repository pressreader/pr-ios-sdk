...
/**
 If thumbnail is not downloaded or didn't start downloading yet, with `priority` set to YES this method removes current thumb from downloading
 stack and put it on the top of it. In this way we achieve the functionality of downloading visible thumbnails first.
...
/// Ignore usage of smaller image if available while required image is loading. NO by default.
@property (nonatomic) BOOL ignoreClosestImageAsPlaceholder;
...
@property (nullable, atomic, strong, readonly) UIImage *image;
